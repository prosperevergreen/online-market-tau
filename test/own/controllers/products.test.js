...
const { createProduct, viewProduct, modifyProduct, deleteProduct } = require('../../../controllers/products');
...
describe('Products Controller', () => {
  ...
  describe('viewProduct()', () => {
    it('should respond with "404 Not Found" if product with given productId does not exist', async () => {
      const wrongProductId = product.id
        .split('')
        .reverse()
        .join('');
      await viewProduct(response, wrongProductId, customerUser);
